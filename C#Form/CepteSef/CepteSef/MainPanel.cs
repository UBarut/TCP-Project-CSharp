...
namespace CepteSef
{
    public partial class MainPanel : Form
    {
        ...
        public void RandomFood()
        {
            ...
            SqlDataReader random = adapter.SqlOperations("select top 10 * from Foods order by NEWID()", connection);
            while (random.Read())
            {
        ...
        public void MostViewedFood()
        {
            ...
            SqlDataReader mostView = adapter.SqlOperations("select top 10 * from Foods order by total_views desc", connection);
            while (mostView.Read())
            {
                foodCard = new UC_FoodCardPanel();
        ...
        public void FavoritesFood()
        {
            ...
            SqlDataReader favorite = adapter.SqlOperations("select Food,COUNT(f.ID),Food_Picture,FoodExplanation from Favorites as fav inner join Foods as f on(fav.Favorite_FoodID=f.ID) group by Food,Food_Picture,FoodExplanation ORDER BY COUNT(f.ID) DESC", connection);
            while (favorite.Read())
            {
                foodCard = new UC_FoodCardPanel();
